...
# price <- prices[tokenId == nativeToken, .(tokenId, date, price)]
# fees <- merge(fees, price, by = 'date', all.x = TRUE)
# fees[, FeeIncomeUSD := FeeIncome * price]
...
pos <- getLoansDailyPositions_acala_loan(network, window = 31) 
...
if (network == 'Karura') {
  # get bad debt penalty
  cdp2 <- getLiquidateUnsafeCDP_acala_loan(network, window = 31)
...
# liquid staking fee
addr <- "23M5ttkmR6Kco5p3LFGKMpMv4zvLkKdUQWW1wGGoV8zDX3am"
...
addr <- "23M5ttkmR6KcnvsNJdmYTpLo9xfc54g8uCk55buDfiJPon69"
filter <- ' filter: {accountId: {equalTo: "23M5ttkmR6KcnvsNJdmYTpLo9xfc54g8uCk55buDfiJPon69"}}'
...
if (network=="Acala") print("__Stability Fees currently not accurate for Acala -- Will be fixed soon__")
...
main <- network %+% " " %+% us[1] %+% " Treasury in 23M5ttkmR6Kco5p3LFGKMpMv4zvLkKdUQWW1wGGoV8zDX3am"
dygraph(dat[tokenId == us[1], .(timestamp, balanceUSD)], main = main)  %>% 
    dySeries("balanceUSD", stepPlot = TRUE, fill = TRUE)

if (length(us) > 1) {
  main <- network %+% " " %+% us[2] %+% " Treasury in 23M5ttkmR6Kco5p3LFGKMpMv4zvLkKdUQWW1wGGoV8zDX3am"
...
main <- network %+% " " %+% us[1] %+% " Treasury in 23M5ttkmR6KcnvsNJdmYTpLo9xfc54g8uCk55buDfiJPon69"
dygraph(dat[tokenId == us[1], .(timestamp, balanceUSD)], main = main)  %>% 
    dySeries("balanceUSD", stepPlot = TRUE, fill = TRUE)

if (length(us) > 1) {
  main <- network %+% " " %+% us[2] %+% " Treasury in 23M5ttkmR6KcnvsNJdmYTpLo9xfc54g8uCk55buDfiJPon69"
  dygraph(dat[tokenId == us[2], .(timestamp, balanceUSD)], main = main)  %>% 
      dySeries("balanceUSD", stepPlot = TRUE, fill = TRUE)
}
