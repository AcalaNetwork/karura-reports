...
# window <- 14; network <- "Karura"; endpoint <- karura_swap_endpoint

# swaps_by_day <- getSwapsByDay(endpoint, window)
# pairs_by_day <- rbind(swaps_by_day[, .N, by = pair], data.table(pair="ALL", N = swaps_by_day[, .N]))[order(N, decreasing = TRUE)] %>% 
#   setnames(c("Pair", "Observations"))

swaps <- getSwaps(endpoint, window)
pairs <- rbind(swaps[exclude == FALSE, .N, by = pair], data.table(pair="ALL", N = swaps[, .N]))[order(N, decreasing = TRUE)] %>%
  setnames(c("Pair", "Observations"))
...
[Acala-Swap-Day-Data](https://project.subquery.network/project/rogerjbos/acala-swap-day-data)
[Karura-Swap-Data](https://project.subquery.network/project/rogerjbos/karura-swap-data)
