...
tokenmap <- tokens
tokenmap[, divisor := (as.numeric(substr(as.character(1e20), 1, as.numeric(decimals) + 1))), by = Token]


network <- params$network

unsafe <- getLiquidateUnsafeCDP_acala_loan(network, 365)
...
swaps <- merge(swaps, tokenmap, by.x="token0", by.y="Token", all.x = TRUE) %>% 
  setnames("divisor", "divisor0")
swaps[, Name := NULL]
swaps[, decimals := NULL]
swaps <- merge(swaps, tokenmap, by.x="token1", by.y="Token", all.x = TRUE) %>% 
  setnames("divisor", "divisor1")
swaps[, Name := NULL]
swaps[, decimals := NULL]
swaps <- merge(swaps, tokenmap, by.x="token2", by.y="Token", all.x = TRUE) %>% 
  setnames("divisor", "divisor2")
swaps[, Name := NULL]
swaps[, decimals := NULL]
swaps <- merge(swaps, tokenmap, by.x="token3", by.y="Token", all.x = TRUE) %>% 
  setnames("divisor", "divisor3")
swaps[, Name := NULL]
swaps[, decimals := NULL]

# swaps[amount0 > 0 & is.na(divisor0)]
swaps[amount0 > 0 & is.na(divisor0), divisor0 := 12]
swaps[amount1 > 0 & is.na(divisor1), divisor1 := 12]
swaps[amount2 > 0 & is.na(divisor2), divisor2 := 12]
swaps[amount3 > 0 & is.na(divisor3), divisor3 := 12]

swaps[, float0 := as.numeric(amount0) / divisor0]
swaps[, float1 := as.numeric(amount1) / divisor1]
swaps[, float2 := as.numeric(amount2) / divisor2]
swaps[, float3 := as.numeric(amount3) / divisor3]
...
price <- read_json(path = "/Users/rogerbos/subquery/price/" %+% network %+% ".json", TRUE) %>% as.data.table
price[, oracle := as.numeric(oracle) / 1e18]
...
* R source code [liquidation.Rmd](https://github.com/AcalaNetwork/karura-reports)
