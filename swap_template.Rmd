...
if (params$pair == "ALL") {
  resx <- swaps[date >= max(res$date) - xdays]
} else {
  resx <- swaps[pair1==params$pair | pair2==params$pair | pair3==params$pair & date >= max(res$date) - xdays]
}
...
```{r}
  ...
  dygraph(avgTrade, main = "Trades Per User" %+% txt)  %>% 
    dySeries("Trades per user", stepPlot = TRUE, fill = TRUE, color = getColor(activeUsersStatus)) %>%
    dySeries("Average", color = "blue") 
...
dygraph(fees, main = "Swap Fees" %+% txt)  %>% 
  dySeries("Swap Fees", stepPlot = TRUE, fill = TRUE, color = getColor(activeUsersStatus)) %>%
  dySeries("Average", color = "blue") 
...
```{r}
  ...
  tvl <- swaps2[pair == params$pair, .(date, tvlUSD)] %>% 
    setnames("date","Date")
  ...
  dygraph(tvl, main = "Volume To TVL" %+% txt)  %>% 
    dySeries("Volume To TVL", stepPlot = TRUE, fill = TRUE, color = getColor(volumeToTVLStatus)) %>%
    dySeries("Average", color = "blue") 
