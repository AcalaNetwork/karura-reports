...
```{r}
  ...
  activeUsersStatus <- tail(activeUsers$`Active Users`, 1) / activeUsersAvg
...
```{r}
  ...
  tradesStatus <- tail(trades$Trades, 1) / tradesAvg
...
```{r}
  ...
  avgTradeStatus <- tail(avgTrade$`Trades per user`, 1) / avgTradeAvg
...
```{r}
  ...
  feesStatus <- tail(fees$`Swap Fees`, 1) / feesAvg
...
```{r}
  ...
  tradeVolumeStatus <- tail(tradeVolume$`Trading Volume`, 1) / tradeVolumeAvg
...
```{r}
  ...
  volumeToTVLStatus <- tail(tvl$`Average`, 1) / volumeToTVLAvg
